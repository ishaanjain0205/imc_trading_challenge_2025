...
class TimeTriggeredStrat:
    def __init__(self, trigger_timestamps: List[int]):
        """
        :param trigger_timestamps: A list of timestamps at which we want to fire trades.
        """
        self.name = "TimeTriggeredStrat"
        # Convert list to set for O(1) "in" checks
        self.trigger_timestamps = set(trigger_timestamps)

        # We'll store the previous iteration's mid-price here.
        # It will be None on the very first iteration or if we have no data.
        self.last_price = None

    def load(self, data: dict) -> None:
        """
        Load any saved state from previous iteration (if present).
        """
        if data is not None:
    ...
    def save(self) -> dict:
        """
        Save any state we want to carry over between iterations.
        """
        return {
    ...
    def run_strategy(
        ...
        # 1. Check if the current timestamp is in our trigger set.
        if current_timestamp not in self.trigger_timestamps:
            return []  # If not a trigger time, do nothing.

        # 2. Compute the current mid-price (fallback to using 'last_price' if the book is empty).
        if order_depth.buy_orders and order_depth.sell_orders:
            best_bid = max(order_depth.buy_orders.keys())
            best_ask = min(order_depth.sell_orders.keys())
            mid_price = (best_bid + best_ask) / 2.0
        else:
            # If we really have no data, fallback to last known price or 100
            mid_price = self.last_price if self.last_price is not None else 100

        # 3. Decide whether to buy or sell based on how `last_price` compares to `mid_price`.
        #    Per your instructions:
        #    If the previous tick is LESS (i.e. last_price < current mid_price) => SELL
        #    If the previous tick is GREATER (i.e. last_price > current mid_price) => BUY
        #    We'll do "max available" quantity subject to the position limit.
        ...
        if self.last_price is not None:
            if self.last_price < mid_price:
                ...
                # We can sell up to (position + position_limit)
                qty = position + position_limit  # how many more we can go short
                if qty > 0:
                    # Place a single SELL order near the current mid
                    price = int(round(mid_price))
                    orders.append(Order(product, price, -qty))

            elif self.last_price > mid_price:
                # BUY as many as possible
                # We can buy up to (position_limit - position)
                qty = position_limit - position
                if qty > 0:
                    price = int(round(mid_price))
                    orders.append(Order(product, price, qty))

        # 4. Update last_price for next iteration
...
class FourierStrat:
    ...
    def __init__(self) -> None:
        ...
        self.base_qty = 1
        self.threshold = 5.0  # Minimal price deviation (in price units) that triggers an order.
        
        # We'll store historical mid prices in this list for volatility (scaling) calculations.
        self.prices = []  # initialize empty list

    def load(self, data: dict) -> None:
        # This strategy doesn't hold persistent dynamic state.
        pass
    ...
    def run_strategy(
        ...
        # Use Fourier reconstruction only if we have coefficients for the product.
        ...
        if order_depth.buy_orders and order_depth.sell_orders:
            ...
            mid = reconstruction  # fallback
        
        # Update our price history.
        self.prices.append(mid)
        # Optional: Limit the price history to the last N ticks (e.g., N=50) to avoid unbounded growth.
        if len(self.prices) > 50:
            self.prices = self.prices[-50:]

        orders: List["Order"] = []
        
        # Trading logic: if mid < reconstruction, expect reversion upward (buy); if mid > reconstruction, expect reversion downward (sell).
        # The order quantity is scaled by volatility computed over recent prices.
        # For buy orders:
        if len(self.prices) > 0:
            ...
            scaling = 1.0
        if mid < reconstruction - std and position < position_limit:
            available = position_limit - position  # available capacity to buy.
            diff = reconstruction - mid
            # Compute scaling factor based on standard deviation of recent prices.
            if len(self.prices) > 0:
                avg = sum(self.prices) / len(self.prices)
                variance = sum((p - avg) ** 2 for p in self.prices) / len(self.prices)
                std = variance ** 0.5
                ...
                scaling = 1.0
            qty = int(min(available, round(self.base_qty * scaling)))
            qty = max(qty, 1)
            orders.append(Order(product, round(mid), qty))
        elif mid > reconstruction-std and position > -position_limit:
            available = position + position_limit  # available capacity to sell.
