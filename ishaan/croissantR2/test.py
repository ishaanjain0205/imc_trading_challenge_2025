...
class LoopingCroissantShorter:
    def __init__(self) -> None:
        self.name = "LoopingCroissantLonger"
        self.croissant_limit = 250  # Target long position (+250)
        self.flip = True  # True = long to limit, False = revert to 0

    def load(self, data: Dict) -> None:
        self.flip = data.get("flip", True)

    def save(self) -> Dict:
        return {"flip": self.flip}
    ...
    def run_strategy(
        ...
        # Correct mid-price calculation: average of best bid and best ask.
        if order_depth.buy_orders and order_depth.sell_orders:
            mid = (max(order_depth.buy_orders) + min(order_depth.sell_orders)) / 2
        else:
            return orders  # skip if no valid price

        if self.flip:
            # --- LONG to limit ---
            # Calculate the order required to reach a target long position of +250.
            # For example, if position is 0, you want to buy 250; if position is already 100, then you need to buy 150.
            order_qty = self.croissant_limit - position
            if order_qty != 0:
                orders.append(Order("CROISSANTS", int(mid), order_qty))
                print(f"[LONG] Sending order for {order_qty} CROISSANTS at {int(mid)}")
        else:
            # --- Revert back to neutral ---
            # To revert to position 0, order is simply the negative of your current position.
            revert_qty = -position
            if revert_qty != 0:
                orders.append(Order("CROISSANTS", int(mid), revert_qty))
                print(f"[REVERT] Reverting position by {revert_qty} CROISSANTS at {int(mid)}")

        # Toggle for the next tick
        self.flip = not self.flip
        return orders



class BasicBasketArbitrage:
    def __init__(self) -> None:
        self.name = "BasicBasketArbitrage"
        self.entry_threshold_pb1 = 10
        self.entry_threshold_pb2 = 8
        self.exit_threshold = 13.5  # Exit threshold for reversion
        
        self.std_dev_factor = 0.5
        self.default_threshold_pb2 = 5
        self.default_threshold_pb1 = 10
        self.spread_history_pb1 = []
        self.spread_history_pb2 = []
    ...
    def load(self, data: dict) -> None:
        self.spread_history_pb1 = data.get("spread_history_pb1", [])
        self.spread_history_pb2 = data.get("spread_history_pb2", [])
        pass

    def save(self) -> dict:
        return {"spread_history_pb1": list(self.spread_history_pb1), "spread_history_pb2": list(self.spread_history_pb2)}
        # return {}
    ...
    def run_strategy(self, state: TradingState) -> List[Order]:
        ...
        mids = {sym: mid(sym) for sym in self.position_limits}
        ...
        # Compute diffs
        diff_pb1 = mids["PICNIC_BASKET1"] - (6 * mids["CROISSANTS"] + 3 * mids["JAMS"] + mids["DJEMBES"])
        diff_pb2 = mids["PICNIC_BASKET2"] - (4 * mids["CROISSANTS"] + 2 * mids["JAMS"])

        # Current positions
        pos_pb1 = pos.get("PICNIC_BASKET1", 0)
        pos_pb2 = pos.get("PICNIC_BASKET2", 0)
        pos_C = pos.get("CROISSANTS", 0)
        pos_J = pos.get("JAMS", 0)
        pos_D = pos.get("DJEMBES", 0)

        # === EXIT MODE ===
        if abs(diff_pb1) < self.exit_threshold or abs(diff_pb2) < self.exit_threshold:
            # Unwind PB1
            if pos_pb1 != 0:
                orders.append(Order("PICNIC_BASKET1", int(mids["PICNIC_BASKET1"]), -pos_pb1))
                orders.append(Order("CROISSANTS", int(mids["CROISSANTS"]), 6 * pos_pb1))
                orders.append(Order("JAMS", int(mids["JAMS"]), 3 * pos_pb1))
                orders.append(Order("DJEMBES", int(mids["DJEMBES"]), 1 * pos_pb1))

            # Unwind PB2
            if pos_pb2 != 0:
                orders.append(Order("PICNIC_BASKET2", int(mids["PICNIC_BASKET2"]), -pos_pb2))
                orders.append(Order("CROISSANTS", int(mids["CROISSANTS"]), 4 * pos_pb2))
                orders.append(Order("JAMS", int(mids["JAMS"]), 2 * pos_pb2))

            # Fully flatten underlying positions (market orders)
            if pos_C != 0:
                orders.append(Order("CROISSANTS", int(mids["CROISSANTS"]), -pos_C))
            if pos_J != 0:
                orders.append(Order("JAMS", int(mids["JAMS"]), -pos_J))
            if pos_D != 0:
                orders.append(Order("DJEMBES", int(mids["DJEMBES"]), -pos_D))

            return orders
        if len(self.spread_history_pb1) >= 5:  # need a few points
            mean_pb1 = np.mean(self.spread_history_pb1)
            std_pb1 = np.std(self.spread_history_pb1)
            # You can define a *band* for over- / under-pricing
            # e.g. "buy if diff < mean - X*std" and "sell if diff > mean + X*std"
            dynamic_low_pb1 = mean_pb1 - self.std_dev_factor * std_pb1
            dynamic_high_pb1 = mean_pb1 + self.std_dev_factor * std_pb1
        else:
            # Not enough data for dynamic; use a default
            dynamic_low_pb1 = -self.default_threshold_pb1
            dynamic_high_pb1 = self.default_threshold_pb1
        
        if len(self.spread_history_pb2) >= 5:
            mean_pb2 = np.mean(self.spread_history_pb2)
            std_pb2 = np.std(self.spread_history_pb2)
            dynamic_low_pb2 = mean_pb2 - self.std_dev_factor * std_pb2
            dynamic_high_pb2 = mean_pb2 + self.std_dev_factor * std_pb2
        else:
            dynamic_low_pb2 = -self.default_threshold_pb2
            dynamic_high_pb2 = self.default_threshold_pb2
            
        # === PB1 ENTRY ===
        if diff_pb1 < dynamic_low_pb1:
            if pos_pb1 < lim["PICNIC_BASKET1"] and pos_C + 6 <= lim["CROISSANTS"] and \
               pos_J + 3 <= lim["JAMS"] and pos_D + 1 <= lim["DJEMBES"]:
                orders.append(Order("PICNIC_BASKET1", int(mids["PICNIC_BASKET1"]), 1))
                orders.append(Order("CROISSANTS", int(mids["CROISSANTS"]), -6))
                orders.append(Order("JAMS", int(mids["JAMS"]), -3))
                orders.append(Order("DJEMBES", int(mids["DJEMBES"]), -1))
        elif diff_pb1 > dynamic_high_pb1:
            if pos_pb1 > -lim["PICNIC_BASKET1"] and pos_C - 6 >= -lim["CROISSANTS"] and \
               pos_J - 3 >= -lim["JAMS"] and pos_D - 1 >= -lim["DJEMBES"]:
                orders.append(Order("PICNIC_BASKET1", int(mids["PICNIC_BASKET1"]), -1))
                orders.append(Order("CROISSANTS", int(mids["CROISSANTS"]), 6))
        ...
        # === PB2 ENTRY ===
        if diff_pb2 < dynamic_low_pb2:
            if pos_pb2 < lim["PICNIC_BASKET2"] and pos_C + 4 <= lim["CROISSANTS"] and \
               pos_J + 2 <= lim["JAMS"]:
                orders.append(Order("PICNIC_BASKET2", int(mids["PICNIC_BASKET2"]), 1))
                orders.append(Order("CROISSANTS", int(mids["CROISSANTS"]), -4))
                orders.append(Order("JAMS", int(mids["JAMS"]), -2))
        elif diff_pb2 > dynamic_high_pb2:
            if pos_pb2 > -lim["PICNIC_BASKET2"] and pos_C - 4 >= -lim["CROISSANTS"] and \
               pos_J - 2 >= -lim["JAMS"]:
                orders.append(Order("PICNIC_BASKET2", int(mids["PICNIC_BASKET2"]), -1))
                orders.append(Order("CROISSANTS", int(mids["CROISSANTS"]), 4))
...
class UnifiedDynamicRollingStrat:
    def __init__(self) -> None:
        self.name = "UnifiedDynamicRollingStrat"
        self.window = 50
        self.pb1_history = deque(maxlen=self.window)
        self.pb2_history = deque(maxlen=self.window)

    def load(self, data: dict) -> None:
        self.pb1_history = deque(data.get("pb1", []), maxlen=self.window)
        self.pb2_history = deque(data.get("pb2", []), maxlen=self.window)

    def save(self) -> dict:
        return {"pb1": list(self.pb1_history), "pb2": list(self.pb2_history)}

    def linear_predict(self, X: np.ndarray, y: np.ndarray, x_now: np.ndarray) -> float:
        X = np.column_stack((np.ones(X.shape[0]), X))
        beta = np.linalg.pinv(X.T @ X) @ X.T @ y
        x_now = np.insert(x_now, 0, 1)
        return float(x_now @ beta)
    ...
    def run_strategy(self, state: TradingState) -> List[Order]:
        ...
        # === PB1 ===
        if all(p in od for p in ["PICNIC_BASKET1", "CROISSANTS", "JAMS", "DJEMBES"]):
            pb1_mid = mid_price(od["PICNIC_BASKET1"])
            C = mid_price(od["CROISSANTS"])
            J = mid_price(od["JAMS"])
            D = mid_price(od["DJEMBES"])
            if None not in [pb1_mid, C, J, D]:
                self.pb1_history.append((C, J, D, pb1_mid))
                if len(self.pb1_history) == self.window:
                    hist = np.array(self.pb1_history)
                    X = hist[:, :3]
                    y = hist[:, 3]
                    x_now = np.array([C, J, D])
                    pred = self.linear_predict(X, y, x_now)
                    diff = pred - pb1_mid
                    std = np.std(y - (np.column_stack((np.ones(X.shape[0]), X)) @ np.linalg.pinv(np.column_stack((np.ones(X.shape[0]), X)).T @ np.column_stack((np.ones(X.shape[0]), X))) @ np.column_stack((np.ones(X.shape[0]), X)).T @ y))
                    if std > 0 and abs(diff / std) > 0.5:
                        scale = min(abs(diff / (2 * std)), 1.0)
                        target = int(np.sign(diff) * scale * 108)
                        current = pos.get("PICNIC_BASKET1", 0)
                        step = max(min(target - current, 5), -5)
                        if step != 0:
                            orders.append(Order("PICNIC_BASKET1", int(pb1_mid), step))

        # === PB2 ===
        if all(p in od for p in ["PICNIC_BASKET2", "CROISSANTS", "JAMS"]):
            pb2_mid = mid_price(od["PICNIC_BASKET2"])
            C = mid_price(od["CROISSANTS"])
            J = mid_price(od["JAMS"])
            if None not in [pb2_mid, C, J]:
                self.pb2_history.append((C, J, pb2_mid))
                if len(self.pb2_history) == self.window:
                    hist = np.array(self.pb2_history)
                    X = hist[:, :2]
                    y = hist[:, 2]
                    x_now = np.array([C, J])
                    pred = self.linear_predict(X, y, x_now)
                    ...
                    if std > 0 and abs(diff / std) > 0.5:
                        scale = min(abs(diff / (2 * std)), 1.0)
                        target = int(np.sign(diff) * scale * 180)
                        current = pos.get("PICNIC_BASKET2", 0)
                        step = max(min(target - current, 5), -5)
                        if step != 0:
                            orders.append(Order("PICNIC_BASKET2", int(pb2_mid), step))
...
class arbDynamic:
    ...
    def run_strategy(self, state: TradingState) -> List[Order]:
        ...
        Otherwise, the strategy computes the expected price for PICNIC_BASKET1 using:
             Expected_B1 = mid_PICNIC_BASKET2 + 2*(mid_CROISSANTS) + (mid_JAMS) + (mid_DJEMBES)
        ...
        products to "build" the basket:
            - PICNIC_BASKET1 consists of: 6 CROISSANTS, 3 JAMS, 1 DJEMBES.
        """
        ...
        required = ["PICNIC_BASKET1", "PICNIC_BASKET2", "CROISSANTS", "JAMS", "DJEMBES"]
        for key in required:
            if key not in state.order_depths:
                return []
        ...
        mid_J  = get_mid(state.order_depths["JAMS"])
...
class DjembeStrat:
    def __init__(self, window_size: int = 10) -> None:
        ...
        self.symbol = "DJEMBES"  # Expect only KELP data here.
...
class Trader:
    def __init__(self) -> None:
        ...
        self.product_strategies: Dict[str, Any] = {
            ...
            "CROISSANTS": LoopingCroissantShorter()
            # "DJEMBES": DjembeStrat()
        }
        ...
        # self.arb_strategies["PB_Regression"] = UnifiedDynamicRollingStrat()
        # self.arb_strategies["BASKET_ARBITRAGE"] = BasicBasketArbitrage()
