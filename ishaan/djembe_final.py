...
class DjembeStrat:
    ...
    def run_strategy(self, product: str, order_depth: OrderDepth, position: int, position_limit: int, timestamp: int) -> List[Order]:
        ...
        # 2.2
        k = 2.1
        adaptive_threshold = min(k * volatility, 2.0)  # Cap so we still trade
        tick_size = 1
        spread_adjustment = volatility + tick_size

        # SCALE quantity based on trend strength
        trend_strength = abs(trend_signal) / adaptive_threshold
        max_base_qty = 15
        quantity = min(int(trend_strength * max_base_qty), position_limit)

        # MOMENTUM branch
        if abs(trend_signal) > adaptive_threshold:
            if trend_signal > 0 and position < position_limit:
                bid_price = int(best_bid)
                max_buy = min(quantity, position_limit - position)
                if max_buy > 0:
                    orders.append(Order(product, bid_price, max_buy))

            elif trend_signal < 0 and position > -position_limit:
        ...
        # MEAN REVERSION fallback
            ...
            alpha = 0.4
            if abs(deviation) > adaptive_threshold:
                if deviation > 0 and position > -position_limit:
                    ask_price = int(current_price + spread_adjustment - alpha * deviation)
                    bid_price = int(current_price - spread_adjustment)
                    max_sell = min(quantity + int(deviation), position + position_limit)
                    if max_sell > 0:
                ...
                elif deviation < 0 and position < position_limit:
                    bid_price = int(current_price - spread_adjustment + alpha * (-deviation))
                    ask_price = int(current_price + spread_adjustment)
                    max_buy = min(quantity + int(-deviation), position_limit - position)
                    if max_buy > 0:
                ...
                if position < position_limit:
                    orders.append(Order(product, bid_price, min(quantity, position_limit - position)))
                if position > -position_limit:
                    orders.append(Order(product, ask_price, -min(quantity, position + position_limit)))
...
class CroissantStrat:
    def __init__(self) -> None:
        self.name = "CroissantVWAPStrat"
        self.mid_history = deque(maxlen=30)
        self.vwap_short_history = deque(maxlen=15)
        self.vwap_long_history = deque(maxlen=60)
        self.std_multiplier = 1.6
        self.trade_size = 5

    def load(self, data: dict) -> None:
        self.mid_history = deque(data.get("mid_history", []), maxlen=30)
        self.vwap_short_history = deque(data.get("vwap_short_history", []), maxlen=15)
        self.vwap_long_history = deque(data.get("vwap_long_history", []), maxlen=60)

    def save(self) -> dict:
        return {
            "mid_history": list(self.mid_history),
            "vwap_short_history": list(self.vwap_short_history),
            "vwap_long_history": list(self.vwap_long_history),
        }
    ...
    def run_strategy(self, product: str, order_depth, position: int, position_limit: int, timestamp: int) -> List[Order]:
        ...
        # Compute VWAP over top 3 levels
        def compute_vwap(order_book: dict) -> float:
            levels = sorted(order_book.items(), key=lambda x: -x[0] if order_book is order_depth.buy_orders else x[0])
            total_vol = 0
            total_px = 0
            for price, qty in levels[:3]:
                total_vol += abs(qty)
                total_px += price * abs(qty)
            return total_px / total_vol if total_vol else mid_price

        vwap = (compute_vwap(order_depth.buy_orders) + compute_vwap(order_depth.sell_orders)) / 2

        self.mid_history.append(mid_price)
        self.vwap_short_history.append(vwap)
        self.vwap_long_history.append(vwap)

        if len(self.mid_history) < 15:
            return orders  # warmup

        spread_std = pstdev([mid - vw for mid, vw in zip(self.mid_history, self.vwap_short_history)])
        upper_bound = vwap + self.std_multiplier * spread_std
        lower_bound = vwap - self.std_multiplier * spread_std

        vwap_short = sum(self.vwap_short_history) / len(self.vwap_short_history)
        vwap_long = sum(self.vwap_long_history) / len(self.vwap_long_history)
        trending_up = vwap_short > vwap_long
        trending_down = vwap_short < vwap_long

        # Trade logic
        if mid_price > upper_bound and position > -position_limit and trending_down:
            sell_qty = min(position + position_limit, self.trade_size)
            orders.append(Order(product, best_bid, -sell_qty))

        elif mid_price < lower_bound and position < position_limit and trending_up:
            buy_qty = min(position_limit - position, self.trade_size)
            orders.append(Order(product, best_ask, buy_qty))

        elif lower_bound < mid_price < upper_bound:
            # Exit if inside band
            if position > 0:
