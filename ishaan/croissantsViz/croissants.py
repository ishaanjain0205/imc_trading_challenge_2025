...
class DjembeStrat:
    ...
    def run_strategy(self, product: str, order_depth: OrderDepth, position: int, position_limit: int, timestmap: int) -> List[Order]:
        ...
        k = 1.9 # factor for adaptive threshold
        adaptive_threshold = k * volatility
        tick_size = 1
        spread_adjustment = volatility + tick_size
        quantity = 5  # base order size
...
class CroissantStrat:
    ...
    def load(self, data: dict) -> None:
        ...
        self.vwap_history = data.get("vwap_history", [])
    ...
    def run_strategy(self, product: str, order_depth: OrderDepth, position: int,
                     position_limit: int, timestamp: int) -> List[Order]:
        orders: List[Order] = []
        ...
        # VWAP calculation (top 3 levels)
        levels = 3
        total_vol = 0
        vwap_numerator = 0

        for price, volume in sorted(order_depth.buy_orders.items(), key=lambda x: -x[0])[:levels]:
            total_vol += volume
            vwap_numerator += price * volume

        for price, volume in sorted(order_depth.sell_orders.items(), key=lambda x: x[0])[:levels]:
            total_vol += abs(volume)
            vwap_numerator += price * abs(volume)

        vwap = vwap_numerator / total_vol if total_vol > 0 else mid_price
        ...
        # Calculate short and long term spread between mid and vwap
        short_spreads = [p - v for p, v in zip(self.price_history[-self.window_short:], self.vwap_history[-self.window_short:])]
        long_spreads = [p - v for p, v in zip(self.price_history[-self.window_long:], self.vwap_history[-self.window_long:])]

        if len(short_spreads) < self.window_short or len(long_spreads) < self.window_long:
            return orders

        short_mean = statistics.mean(short_spreads)
        long_mean = statistics.mean(long_spreads)
        trending = abs(short_mean - long_mean) > 0.3  # heuristic threshold

        # Use long-term std for stable bound, can be tuned
        spread_std = statistics.pstdev(long_spreads)
        upper_bound = vwap + (spread_std * self.std_multiplier)
        lower_bound = vwap - (spread_std * self.std_multiplier)

        # Adjust order size if trending (more aggressive)
        base_qty = 10
        size = base_qty * 2 if trending else base_qty

        if mid_price > upper_bound and position > -position_limit:
            sell_qty = min(position + position_limit, size)
            orders.append(Order(product, best_bid, -sell_qty))

        elif mid_price < lower_bound and position < position_limit:
            buy_qty = min(position_limit - position, size)
