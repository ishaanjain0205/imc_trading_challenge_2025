...
class KelpAlgo:
    def __init__(self, params: Dict[str, Any]):
        ...
        self.alpha = params["alpha"]
        self.position_limit = params["position_limit"]
        self.base_quantity = params["base_quantity"]
        self.lower_bound = params["LOWER_BOUND"]
        self.upper_bound = params["UPPER_BOUND"]
        self.bound_tolerance = params["BOUND_TOLERANCE"]
        self.max_history = params["MAX_HISTORY"]
        self.history: List[float] = []
        self.fair_value: float = params["fair_value"]
    ...
    def run(self, current_price: float, order_depth: OrderDepth, position: int) -> List[Order]:
        ...
        if best_bid is not None and best_ask is not None and best_bid > best_ask:
            ...
            max_arbitrage_buy = self.position_limit - position
            max_arbitrage_sell = position + self.position_limit
            arbitrage_volume = min(arbitrage_volume, max_arbitrage_buy, max_arbitrage_sell)
            
            if arbitrage_volume > 0:
                orders.append(Order(Product.KELP, int(best_ask), arbitrage_volume))
                orders.append(Order(Product.KELP, int(best_bid), -arbitrage_volume))
